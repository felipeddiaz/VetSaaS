import { memo, useId, useMemo, useLayoutEffect, useState } from "react";
import {
  AreaChart, Area,
    ...
    <div className="chart-tooltip">
      ...
      {payload.map((p) => (
        <div key={p.dataKey} className="chart-tooltip-row">
          <span className="chart-tooltip-dot" style={{ "--dot-color": p.stroke }} />
          <span>{p.name}</span>
...
function RevenueLineChart({ rangeDays = 30 }) {
  ...
  const [colors, setColors] = useState({ primary: "#1a4434", accent: "#d67b5c" });
  useLayoutEffect(() => {
    setColors({
